...
.metric-card:hover { border-color: rgba(0,200,180,0.25); }
...
.metric-sub { font-size: 12px; color: #889aaa; margin-top: 6px; }
...
.section-sub { font-size: 12px; color: #556677; margin-bottom: 18px; }
...
# load the diabetes prevalence CSV — path goes up one level to the Data folder
csv_path = os.path.join(os.path.dirname(__file__), "..", "Data",
    "cfafrica-data-team-outbreak-covid19-data-openafrica-uploads-kenya-diabetes (1).csv")
df = pd.read_csv(csv_path)

# clean up county names to match the geojson
...
st.markdown('<div class="section-title">Prevalence Map</div>', unsafe_allow_html=True)
st.markdown('<div class="section-sub">Hover over a county to see its diabetes rate</div>', unsafe_allow_html=True)

# pull kenya county boundaries from github — dissolve handles duplicate geometries
...
fig_all.add_hline(
    y=avg_prev, line_dash="dot", line_color="#f39c12",
...
with col_a:
    ...
    st.markdown('<div class="section-sub">Histogram with violin overlay</div>', unsafe_allow_html=True)
