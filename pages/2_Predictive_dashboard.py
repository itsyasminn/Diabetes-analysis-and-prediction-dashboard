...
# feature importances for the bar chart
...
# using sqlite so predictions persist within a session
...
# handle existing databases that don't have the name column yet
try:
...
# shared plotly theme — reused across all charts
PLOTLY_LAYOUT = dict(
...
with st.sidebar:
    ...
    st.markdown(
        '<div class="input-hint">Adjust the sliders to match patient measurements, then click <b>Run Prediction</b>.</div>',
        unsafe_allow_html=True
    )
    ...
    insulin     = st.slider("Insulin (μU/mL)", 0, 100, 100)
...
with left_col:
    if predict_button:
        ...
        name_display = patient_name.strip() if patient_name.strip() else "Unknown"

        c.execute('''INSERT INTO predictions
            (patient_name, pregnancies, glucose, blood_pressure, skin_thickness,
             insulin, bmi, diabetes_pedigree_function, age, prediction, probability, timestamp)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (name_display, pregnancies, glucose, bp, skin, insulin, bmi, dpf, age,
        ...
        <div class="result-card {card_class}">
            <div class="result-label {card_class}"> Prediction Result</div>
            <div class="result-patient">Patient: <span>{name_display}</span></div>
        ...
        fig_gauge.update_layout(
            **{**PLOTLY_LAYOUT, 'margin': dict(l=20, r=20, t=30, b=10)},
            height=280
        )
        ...
        fig_donut = go.Figure(go.Pie(
            ...
            hole=0.6,
            marker=dict(
                colors=["rgba(39,174,96,0.7)", "rgba(231,76,60,0.7)"],
        ...
        fig_donut.update_layout(
            **PLOTLY_LAYOUT,
            ...
            annotations=[dict(
                ...
                font=dict(size=22, color="#e0eaf4", family="Space Mono"),
                showarrow=False
            )],
        )
        ...
        <div style="text-align:center; padding:80px 20px; color:#334455;">
            ...
            <div style="font-size:13px; color:#334455; line-height:1.6;">
                Adjust the patient vitals in the sidebar and click <b style="color:#445566">Run Prediction</b>.
            </div>
        </div>
        """, unsafe_allow_html=True)
...
with right_col:
    ...
    vals = [
        pregnancies / 20, glucose / 200, bp / 150, skin / 100,
...
if df_history.empty:
    st.markdown(
        '<div class="history-empty">No predictions logged yet. Run a prediction to start tracking.</div>',
        unsafe_allow_html=True
    )
else:
    ...
    with col_b:
        ...
        fig_scatter.update_layout(
            **PLOTLY_LAYOUT,
            ...
            height=280,
            title_text="Predictions Over Time",
            title_font=dict(color="#8899aa", size=13),
    ...
    with st.expander("View Full Log"):
        display_cols = ['id', 'patient_name', 'pregnancies', 'glucose', 'blood_pressure',
                        'bmi', 'age', 'prediction', 'probability', 'timestamp']
        df_show = df_history[display_cols].copy()
        df_show['probability'] = (df_show['probability'] * 100).round(1).astype(str) + '%'
        df_show['prediction'] = df_show['prediction'].map({0: 'No Diabetes', 1: 'Diabetes'})
        df_show = df_show.rename(columns={'patient_name': 'Patient Name'})
